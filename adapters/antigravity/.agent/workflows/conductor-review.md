...
2. **Load Antigravity Skills Context:**
   - Check repo-local `.agent/skills/`.
   - Check global skills at `${CODEX_HOME:-$HOME/.agent}/skills/` and `~/.agent/skills/`.
   ...
   - If `conductor/skills/catalog.md` or `${CODEX_HOME:-$HOME/.agent}/conductor/skills/catalog.md` exists, use it only as a recommendation/reference catalog. Do not download external skills unless the user explicitly asks.
...
7. **Skill-Specific Checks:** Apply relevant installed Antigravity skills and catalog recommendations when they match the domain.
