...
2. **Load Claude Code Skills Context:**
   - Check repo-local `.claude/skills/`.
   - Check global skills at `${CODEX_HOME:-$HOME/.claude}/skills/` and `~/.claude/skills/`.
   ...
   - If `conductor/skills/catalog.md` or `${CODEX_HOME:-$HOME/.claude}/conductor/skills/catalog.md` exists, use it only as a recommendation/reference catalog. Do not download external skills unless the user explicitly asks.
...
7. **Skill-Specific Checks:** Apply relevant installed Claude Code skills and catalog recommendations when they match the domain.
