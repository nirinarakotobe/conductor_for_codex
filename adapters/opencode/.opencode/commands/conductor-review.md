...
2. **Load OpenCode Skills Context:**
   - Check repo-local `.opencode/skills/`.
   - Check global skills at `${CODEX_HOME:-$HOME/.opencode}/skills/` and `~/.opencode/skills/`.
   ...
   - If `conductor/skills/catalog.md` or `${CODEX_HOME:-$HOME/.opencode}/conductor/skills/catalog.md` exists, use it only as a recommendation/reference catalog. Do not download external skills unless the user explicitly asks.
...
7. **Skill-Specific Checks:** Apply relevant installed OpenCode skills and catalog recommendations when they match the domain.
